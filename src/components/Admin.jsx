...
export default function Admin({ currentUser, onLogout, tenantId = null }) {
  ...
  const loadAllData = async () => {
    setLoading(true);
    try {
      const apps = await getAppointments(tenantId);
      setAppointments(apps || []);

      const affs = await getAffiliates(tenantId);
      setAffiliates(affs || []);

      const servs = await getServices(tenantId);
      setServices(servs || []);

      const stf = await getStaff(tenantId);
      setStaff(stf || []);

      const promos = await getPromoCodes(tenantId);
      setPromoCodes(promos || []);

      // Load gallery images
      const gallery = await getGalleryImages(tenantId);
      setGalleryImages(gallery || []);
      const cats = await getGalleryCategories(tenantId);
      ...
      // Load hero banners
      const banners = await getHeroBanners(tenantId);
  ...
  const handlePromoteClient = async (id, newRole) => {
    ...
    if (supabase) {
      ...
      if (error) {
        setMessage({ text: "Erreur de mise à jour du réle.", type: "error" });
        return;
      }
    }
  ...
  const handleSaveService = async (e) => {
    ...
    if (serviceImageFile) {
      ...
      const uploaded = await uploadImage(serviceImageFile, "abaia");
    ...
    if (editingService) {
      ...
      setMessage({ text: "Soin mis à jour avec succés !", type: "success" });
    } else {
  ...
  const handleAddGalleryImage = async (e) => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const uploaded = await uploadImage(file, "abaia");
      if (uploaded) {
  ...
  const handleSaveProduct = async (e) => {
    ...
    if (productImageFile) {
      ...
      const uploaded = await uploadImage(productImageFile, "abaia");
      if (uploaded) image_url = uploaded;
    }
    // Upload extra images
    const extraUrls = [];
    for (let i = 0; i < productExtraFiles.length; i++) {
      ...
      const url = await uploadImage(productExtraFiles[i], "abaia");
  ...
  const handleSaveSiteSettings = async (e) => {
    ...
    if (logoFile) {
      ...
      const url = await uploadImage(logoFile, "abaia");
      if (url) updates.logo_url = url;
    }
    if (faviconFile) {
      setMessage({ text: 'Upload favicon...', type: 'info' });
      const url = await uploadImage(faviconFile, "abaia");
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{ 
          ...
          {[
            ...
            { id: "abaia", label: "Galerie", icon: Image },
            ...
            {showServiceForm && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
                <form onSubmit={handleSaveService} className="glass-panel" style={{ maxWidth: "500px", width: "100%", padding: "30px", border: "1px solid var(--primary-gold)", display: "flex", flexDirection: "column", gap: "14px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px", padding: "14px", background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(212,175,55,0.4)", borderRadius: "10px" }}>
                    <label style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 600, textTransform: "uppercase" }}>Image du soin</label>
                    <label style={{ cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px", border: "2px dashed rgba(255,255,255,0.1)", borderRadius: "8px", background: "rgba(0,0,0,0.2)", transition: "all 0.3s" }} onMouseOver={e => e.currentTarget.style.borderColor = "var(--primary-gold)"} onMouseOut={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"}>
                      <input type="file" accept="image/*" onChange={handleServiceImageChange} style={{ display: "none" }} />
                      {(serviceImagePreview || serviceForm.image_url) ? (
                        <img src={serviceImagePreview || serviceForm.image_url} alt="preview" style={{ width: "100%", maxHeight: "180px", objectFit: "contain", borderRadius: "8px" }} />
                      ) : (
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                          <div style={{ background: "rgba(212,175,55,0.1)", padding: "10px", borderRadius: "50%", marginBottom: "10px" }}>
                            <Image size={22} style={{ color: "var(--primary-gold)" }} />
                          </div>
                          <span style={{ fontSize: "0.82rem", color: "var(--text-primary)" }}>Cliquez pour ajouter une image</span>
                          <span style={{ fontSize: "0.68rem", color: "var(--text-secondary)", marginTop: "4px" }}>PNG, JPG, WEBP</span>
                        </div>
                      )}
                    </label>
                  </div>
            ...
            {showProductForm && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
                <form onSubmit={handleSaveProduct} className="glass-panel" style={{ maxWidth: "520px", width: "100%", padding: "32px", border: "1px solid var(--primary-gold)", display: "flex", flexDirection: "column", gap: "16px", maxHeight: "90vh", overflowY: "auto" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "16px", padding: "16px", background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(212,175,55,0.4)", borderRadius: "10px" }}>
                    <h4 style={{ margin: 0, fontSize: "0.9rem", color: "var(--primary-gold)", display: "flex", alignItems: "center", gap: "8px" }}><Image size={18} /> Médias du produit</h4>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      <label style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 600 }}>IMAGE PRINCIPALE</label>
                      <label style={{ cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "30px", border: "2px dashed rgba(255,255,255,0.1)", borderRadius: "8px", background: "rgba(0,0,0,0.2)", transition: "all 0.3s" }} onMouseOver={e => e.currentTarget.style.borderColor = "var(--primary-gold)"} onMouseOut={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"}>
                        ...
                        {productImagePreview || productForm.image_url ? (
                          <img src={productImagePreview || productForm.image_url} alt="preview" style={{ width: "100%", maxHeight: "200px", objectFit: "contain", borderRadius: "8px" }} />
                        ) : (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                            <div style={{ background: "rgba(212,175,55,0.1)", padding: "12px", borderRadius: "50%", marginBottom: "12px" }}>
                              <Image size={24} style={{ color: "var(--primary-gold)" }} />
                            </div>
                            <span style={{ fontSize: "0.85rem", color: "var(--text-primary)" }}>Cliquez pour uploader une image</span>
                            <span style={{ fontSize: "0.7rem", color: "var(--text-secondary)", marginTop: "4px" }}>PNG, JPG, WEBP (max 5MB)</span>
                          </div>
                        )}
                      </label>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "4px" }}>
                        <div style={{ height: "1px", background: "rgba(255,255,255,0.1)", flex: 1 }} />
                        <span style={{ fontSize: "0.7rem", color: "var(--text-secondary)" }}>OU</span>
                        <div style={{ height: "1px", background: "rgba(255,255,255,0.1)", flex: 1 }} />
                      </div>
                      <input type="url" value={productForm.image_url} onChange={e => setProductForm({ ...productForm, image_url: e.target.value })} placeholder="Lien URL de l'image (https://...)" style={{ padding: "10px", background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text-primary)", borderRadius: "6px", outline: "none", fontSize: "0.8rem" }} />
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "8px" }}>
                      <label style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 600 }}>IMAGES SUPPLÉMENTAIRES (Max 7)</label>
                      <label style={{ cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", padding: "12px", border: "1px dashed rgba(255,255,255,0.2)", borderRadius: "6px", background: "rgba(255,255,255,0.03)", transition: "all 0.2s" }} onMouseOver={e => e.currentTarget.style.background = "rgba(212,175,55,0.1)"} onMouseOut={e => e.currentTarget.style.background = "rgba(255,255,255,0.03)"}>
                        <input type="file" accept="image/*" multiple onChange={handleProductExtraImages} style={{ display: "none" }} />
                        <Plus size={16} style={{ color: "var(--primary-gold)" }} />
                        <span style={{ fontSize: "0.8rem", color: "var(--text-primary)" }}>Ajouter des images</span>
                      </label>
                      {productExtraPreviews.length > 0 && (
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(70px, 1fr))", gap: "8px", marginTop: "8px" }}>
                          {productExtraPreviews.map((src, i) => (
                            <div key={i} style={{ position: "relative" }}>
                              ...
                              <button type="button" onClick={() => handleRemoveProductExtra(i)} style={{ position: "absolute", top: "-4px", right: "-4px", background: "#ff4757", border: "none", borderRadius: "50%", width: "20px", height: "20px", color: "#fff", cursor: "pointer", fontSize: "12px", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 4px rgba(0,0,0,0.3)" }}>×</button>
                            </div>
                          ))}
                        </div>
                      )}
            ...
            {showGalleryForm && (
              <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "20px" }}>
                <form onSubmit={handleAddGalleryImage} className="glass-panel" style={{ maxWidth: "520px", width: "100%", padding: "32px", border: "1px solid var(--primary-gold)", display: "flex", flexDirection: "column", gap: "16px", maxHeight: "90vh", overflowY: "auto" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px", padding: "14px", background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(212,175,55,0.4)", borderRadius: "10px" }}>
                    <label style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 600, textTransform: "uppercase" }}>Photos de la galerie</label>
                    <label style={{ cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "28px", border: "2px dashed rgba(255,255,255,0.1)", borderRadius: "8px", background: "rgba(0,0,0,0.2)", transition: "all 0.3s" }} onMouseOver={e => e.currentTarget.style.borderColor = "var(--primary-gold)"} onMouseOut={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"}>
                      <input type="file" accept="image/*" multiple onChange={handleGalleryFilesChange} style={{ display: "none" }} />
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                        <div style={{ background: "rgba(212,175,55,0.1)", padding: "10px", borderRadius: "50%", marginBottom: "10px" }}>
                          <Image size={22} style={{ color: "var(--primary-gold)" }} />
                        </div>
                        <span style={{ fontSize: "0.82rem", color: "var(--text-primary)" }}>Cliquez pour choisir des images</span>
                        <span style={{ fontSize: "0.68rem", color: "var(--text-secondary)", marginTop: "4px" }}>Sélection multiple possible (max 8)</span>
                      </div>
                    </label>
                    {galleryUploadPreviews.length > 0 && (
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(80px, 1fr))", gap: "8px" }}>
                        {galleryUploadPreviews.map((src, i) => (
                          <div key={i} style={{ position: "relative" }}>
                            <img src={src} alt={"img-" + i} style={{ width: "100%", height: "80px", objectFit: "cover", borderRadius: "6px", border: "1px solid rgba(212,175,55,0.3)" }} />
                            <button type="button" onClick={() => handleRemoveGalleryPreview(i)} style={{ position: "absolute", top: "-4px", right: "-4px", background: "#ff4757", border: "none", borderRadius: "50%", width: "20px", height: "20px", color: "#fff", cursor: "pointer", fontSize: "12px", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 4px rgba(0,0,0,0.3)" }}>×</button>
                          </div>
                        ))}
                      </div>
                    )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "30px" }}>
            ...
            <div className="glass-panel" style={{ padding: "30px", border: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <form onSubmit={handleSaveSiteSettings} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 16px", background: siteSettings.promo_banner_active !== false ? "rgba(34,139,34,0.08)" : "rgba(255,69,0,0.06)", borderRadius: "8px", border: siteSettings.promo_banner_active !== false ? "1px solid rgba(34,139,34,0.2)" : "1px solid rgba(255,69,0,0.15)" }}>
                  ...
                  <div>
                    <div style={{ fontSize: "0.9rem", fontWeight: "600", color: siteSettings.promo_banner_active !== false ? "#4CAF50" : "#FF5722" }}>
                      {siteSettings.promo_banner_active !== false ? "Activée — visible sur le site" : "Désactivée — masquée pour les visiteurs"}
                    </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                    <label style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 600, textTransform: "uppercase" }}>Logo</label>
                    <label style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: "12px", padding: "14px", border: "2px dashed rgba(255,255,255,0.1)", borderRadius: "8px", background: "rgba(0,0,0,0.2)", transition: "all 0.3s" }} onMouseOver={e => e.currentTarget.style.borderColor = "var(--primary-gold)"} onMouseOut={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"}>
                      <input type="file" accept="image/*" onChange={e => setLogoFile(e.target.files[0])} style={{ display: "none" }} />
                      {siteSettings.logo_url ? (
                        <img src={siteSettings.logo_url} alt="Logo" style={{ height: "40px", objectFit: "contain" }} />
                      ) : (
                        <div style={{ background: "rgba(212,175,55,0.1)", padding: "8px", borderRadius: "50%" }}><Image size={18} style={{ color: "var(--primary-gold)" }} /></div>
                      )}
                      <div>
                        <span style={{ fontSize: "0.82rem", color: "var(--text-primary)" }}>{logoFile ? logoFile.name : "Cliquez pour uploader"}</span>
                        <span style={{ display: "block", fontSize: "0.68rem", color: "var(--text-secondary)" }}>PNG, SVG recommandé</span>
                      </div>
                    </label>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                    <label style={{ fontSize: "0.75rem", color: "var(--text-secondary)", fontWeight: 600, textTransform: "uppercase" }}>Favicon</label>
                    <label style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: "12px", padding: "14px", border: "2px dashed rgba(255,255,255,0.1)", borderRadius: "8px", background: "rgba(0,0,0,0.2)", transition: "all 0.3s" }} onMouseOver={e => e.currentTarget.style.borderColor = "var(--primary-gold)"} onMouseOut={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"}>
                      <input type="file" accept="image/*" onChange={e => setFaviconFile(e.target.files[0])} style={{ display: "none" }} />
                      {siteSettings.favicon_url ? (
                        <img src={siteSettings.favicon_url} alt="Favicon" style={{ height: "32px", width: "32px", objectFit: "contain" }} />
                      ) : (
                        <div style={{ background: "rgba(212,175,55,0.1)", padding: "8px", borderRadius: "50%" }}><Image size={18} style={{ color: "var(--primary-gold)" }} /></div>
                      )}
                      <div>
                        <span style={{ fontSize: "0.82rem", color: "var(--text-primary)" }}>{faviconFile ? faviconFile.name : "Cliquez pour uploader"}</span>
                        <span style={{ display: "block", fontSize: "0.68rem", color: "var(--text-secondary)" }}>ICO, PNG 32x32px</span>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    for (const file of files) {
                      ...
                      const url = await uploadImage(file, "abaia");
                      if (url) {
                    ...
                    if (count > 0) {
                      ...
                      const updated = await getHeroBanners(tenantId);
